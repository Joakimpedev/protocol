...
const TABLE_FULL_PROTOCOL_HEADER_BG = '#00C200'; // Green background for "Full Protocol" header
const TABLE_FULL_PROTOCOL_CELL_BG = '#00B500'; // Green background for "Full Protocol" row cells

interface PhotoPaywallModalProps {
  visible: boolean;
  onClose: () => void;
  onPurchaseComplete?: () => void;
}

export default function PhotoPaywallModal({
  visible,
  onClose,
  onPurchaseComplete,
    ...
    if (visible) {
      ...
      if (posthog) {
        posthog.capture('paywall_viewed', {
          trigger: 'week_5_photo_attempt',
        });
      }
    }
  }, [visible, posthog]);

  const loadPhotosAndScore = async () => {
    if (!user) return;
    ...
    try {
      ...
      const score = await calculateWeeklyConsistency(user.uid);
      setConsistencyScore(score);
    } catch (error) {
      console.error('Error loading photos and score:', error);
    } finally {
      setLoadingData(false);
    }
  };

  const formatConsistencyScore = (score: number): string => {
    // Score is 0.0-10.0 scale, convert to percentage (0-100%)
  ...
  // Pan responder for slider drag (same as PhotoComparisonScreen)
            ...
            <View style={styles.titleSection}>
              ...
              {consistencyScore !== null && (
                <Text style={styles.consistencyScore}>
                  {formatConsistencyScore(consistencyScore)} consistency
                </Text>
              )}
            </View>
